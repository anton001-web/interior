export const worksPortfolioTypes = [
    {
        title: 'Квартиры',
        type: 'flats'
    },
    {
        title: 'Коттеджи',
        type: 'cottages'
    },
    {
        title: 'Коммерческая',
        type: 'commercial'
    }
]

export const worksPortfolioItems = [
    {
        type: 'flats',
        worksList: [
            {
                img: './assets/images/FlatWorksImage1.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork1',
            },
            {
                img: './assets/images/FlatWorksImage2.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork2',
            },
            {
                img: './assets/images/FlatWorksImage3.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork3',
            },
            {
                img: './assets/images/FlatWorksImage4.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork4',
            },
            {
                img: './assets/images/FlatWorksImage5.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork5',
            },
            {
                img: './assets/images/FlatWorksImage6.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork5',
            },
            {
                img: './assets/images/FlatWorksImage7.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork6',
            },
            {
                img: './assets/images/FlatWorksImage8.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork7',
            },
            {
                img: './assets/images/FlatWorksImage9.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork8',
            },
            {
                img: './assets/images/FlatWorksImage10.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork9',
            },
            {
                img: './assets/images/FlatWorksImage1.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork10',
            },
            {
                img: './assets/images/FlatWorksImage2.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork11',
            },
            {
                img: './assets/images/FlatWorksImage3.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork12',
            },
        ]
    },
    {
        type: 'cottages',
        worksList: [
            {
                img: './assets/images/FlatWorksImage1.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork13',
            },
            {
                img: './assets/images/FlatWorksImage3.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork14',
            },
            {
                img: './assets/images/FlatWorksImage2.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork15',
            },
            {
                img: './assets/images/FlatWorksImage6.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork16',
            },
            {
                img: './assets/images/FlatWorksImage5.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork17',
            },
            {
                img: './assets/images/FlatWorksImage4.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork18',
            },
            {
                img: './assets/images/FlatWorksImage7.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork19',
            },
            {
                img: './assets/images/FlatWorksImage8.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork20',
            },
            {
                img: './assets/images/FlatWorksImage9.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork21',
            },
            {
                img: './assets/images/FlatWorksImage10.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork22',
            },
            {
                img: './assets/images/FlatWorksImage1.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork23',
            },
            {
                img: './assets/images/FlatWorksImage2.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork24',
            },
            {
                img: './assets/images/FlatWorksImage3.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork25',
            },
        ]
    },
    {
        type: 'commercial',
        worksList: [
            {
                img: './assets/images/FlatWorksImage1.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork26',
            },
            {
                img: './assets/images/FlatWorksImage2.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork27',
            },
            {
                img: './assets/images/FlatWorksImage3.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork28',
            },
            {
                img: './assets/images/FlatWorksImage4.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork29',
            },
            {
                img: './assets/images/FlatWorksImage5.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork30',
            },
            {
                img: './assets/images/FlatWorksImage6.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork31',
            },
            {
                img: './assets/images/FlatWorksImage7.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork32',
            },
            {
                img: './assets/images/FlatWorksImage8.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork33',
            },
            {
                img: './assets/images/FlatWorksImage9.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork34',
            },
            {
                img: './assets/images/FlatWorksImage10.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork35',
            },
            {
                img: './assets/images/FlatWorksImage1.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork36',
            },
            {
                img: './assets/images/FlatWorksImage2.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork37',
            },
            {
                img: './assets/images/FlatWorksImage3.png',
                title: 'ЖК Триумф Парк',
                to: '/portfolioWork38',
            },
        ]
    }
]

export const portfolioWorksList = [
    {
        flatSquare: '105',
        title: 'ЖК Триумф Парк',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork1',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork2',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork3',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork4',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork5',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork6',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork7',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork8',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork9',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork10',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork11',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork12',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork13',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork14',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork15',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork16',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork17',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork18',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork19',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork20',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork21',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork22',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork23',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork24',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork25',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork26',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork27',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork28',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork29',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork30',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork31',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork32',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork33',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork34',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork35',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork36',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork37',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    },
    {
        flatSquare: '105',
        aboutProject: 'Это очень интересный для нас проект. Заказчики обратились к нам за дизайн-проектом их квартиры, которая оказалась не в новом ЖК, а во вполне обычном пятиэтажном доме. Они решили полностью обновить интерьер квартиры и вдохнуть в нее вторую жизнь! Мы откликнулись и приложили все усилия, чтобы эта небольшая трешка заиграла новыми красками!',
        exercises: 'Создать стильный и эргономичный интерьер квартиры для семьи из четырех человек и собаки.',
        type: 'portfolioWork38',
        title: 'ЖК Триумф Парк',
        heroImg: './assets/images/portfWorkHeroImg.png',
        mainImg: './assets/images/portfWorkMainImg.png',
        firstImgList: [
            "./assets/images/portfWorkImgsList1.png",
            "./assets/images/portfWorkImgsList2.png",
            "./assets/images/portfWorkImgsList3.png",
            "./assets/images/portfWorkImgsList4.png",
            "./assets/images/portfWorkImgsList5.png",
            "./assets/images/portfWorkImgsList6.png",
            "./assets/images/portfWorkImgsList7.png",
            "./assets/images/portfWorkImgsList8.png"
        ],
        secondImgList: [
            "./assets/images/portfSecondWorkList1.png",
            "./assets/images/portfSecondWorkList2.png",
            "./assets/images/portfSecondWorkList3.png",
            "./assets/images/portfSecondWorkList4.png",
        ],
        similarProjectsList: [
            {
                img: './assets/images/similarProject1.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject2.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject3.png',
                title: 'ЖК Триумф Парк'
            },
            {
                img: './assets/images/similarProject4.png',
                title: 'ЖК Триумф Парк'
            }
        ],
    }
]